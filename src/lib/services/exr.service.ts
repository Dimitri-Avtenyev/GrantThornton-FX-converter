...
const getEurRates = async (
  ...
  const response = await fetch(
    `${entryPoint}?startPeriod=${startPeriod}&endPeriod=${endPeriod}&format=${format}&detail=dataonly`,
  );
...
const exchangeRateDictBuilderECB = async (
  data: ExchangeRateData,
): Promise<ExchangeRateDict> => {
  let eurRates: ExchangeRateDict = {};
  let observations: number =
    data.structure.dimensions.observation[0].values.length;
  ...
  for (let i = 0; i < observations; i++) {
    for (let j = 0; j < dataSetSeries; j++) {
      ...
      if (
        data.dataSets[0].series[`0:${j}:0:0:0`] &&
        data.dataSets[0].series[`0:${j}:0:0:0`].observations &&
        data.dataSets[0].series[`0:${j}:0:0:0`].observations[i]
      ) {
        ...
        let isoDate: string =
          data.structure.dimensions.observation[0].values[i].id;
