...
export const populateLocalDB = async (): Promise<boolean> => {
  ...
  if (jsonFile) {
    let data: string = await fs.readFile(
      `${localDataPath}/${jsonFile}`,
      'utf-8',
    );
    let eurRatesJson: ExchangeRateDict = {};
    // todo fix flow of deleting file
    try {
      eurRatesJson = JSON.parse(data);
    } catch {
      fs.unlink(`${localDataPath}/${jsonFile}`);
      console.log('Something wrong with the file, replacing file... ');
      let rates: ExchangeRateDict = await exrService.getEurRates(
        startPeriod,
        endPeriod,
      );
    ...
    if (
      Object.keys(eurRatesJson).length > 700 &&
      diff > twoYearsInMilliseconds
    ) {
      try {
